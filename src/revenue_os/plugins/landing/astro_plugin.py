...
class AstroLandingPlugin(BasePlugin):
    """
    AstroLandingPlugin (Sprint O4).
    Plugin de Geração e Deploy de Landing Pages utilizando Astro SSG.
    """
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        cdn = payload.get("cdn_provider", "cloudflare_pages")

        if action == "build":
            build_res = self.engine.build_landing(payload, framework="astro")
            ...
            return {
                ...
                "built_with": "Astro v5 SSG + Tailwind CSS",
        ...
        raise ValueError(f"Ação desconhecida '{action}' para AstroLandingPlugin")
