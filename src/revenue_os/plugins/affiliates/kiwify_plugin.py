...
from src.revenue_os.plugins.affiliates.providers.kiwify_provider import KiwifyProvider
from src.revenue_os.plugins.affiliates.models import DeepLinkRequest

class KiwifyAffiliatePlugin(BasePlugin):
    """
    KiwifyAffiliatePlugin (Sprint O2).
    Plugin estendendo BasePlugin SDK para integração total com Kiwify.
    """

    def __init__(self, provider: Optional[KiwifyProvider] = None):
        self.provider = provider or KiwifyProvider()
        self._initialized = False

    @property
    def plugin_name(self) -> str:
        return "kiwify_affiliate_plugin"
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        raise ValueError(f"Ação desconhecida '{action}' para KiwifyAffiliatePlugin")
