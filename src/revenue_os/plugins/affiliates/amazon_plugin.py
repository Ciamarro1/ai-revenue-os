...
from src.revenue_os.plugins.affiliates.providers.amazon_provider import AmazonProvider
from src.revenue_os.plugins.affiliates.models import DeepLinkRequest

class AmazonAffiliatePlugin(BasePlugin):
    """
    AmazonAffiliatePlugin (Sprint O2).
    Plugin estendendo BasePlugin SDK para integração total com Amazon Associates.
    """

    def __init__(self, provider: Optional[AmazonProvider] = None):
        self.provider = provider or AmazonProvider()
        self._initialized = False

    @property
    def plugin_name(self) -> str:
        return "amazon_affiliate_plugin"
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        raise ValueError(f"Ação desconhecida '{action}' para AmazonAffiliatePlugin")
