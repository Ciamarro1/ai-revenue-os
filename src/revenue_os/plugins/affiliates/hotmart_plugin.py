...
from src.revenue_os.plugins.affiliates.providers.hotmart_provider import HotmartProvider
from src.revenue_os.plugins.affiliates.models import DeepLinkRequest

class HotmartAffiliatePlugin(BasePlugin):
    """
    HotmartAffiliatePlugin (Sprint O2).
    Plugin estendendo BasePlugin SDK para integração total com Hotmart.
    """

    def __init__(self, provider: Optional[HotmartProvider] = None):
        self.provider = provider or HotmartProvider()
        self._initialized = False

    @property
    def plugin_name(self) -> str:
        return "hotmart_affiliate_plugin"
    ...
    def execute(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        raise ValueError(f"Ação desconhecida '{action}' para HotmartAffiliatePlugin")
