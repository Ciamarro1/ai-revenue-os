...
class RemotionVideoProvider(BaseVideoProvider):
    """
    Provedor Gerativo Remotion (Composição de vídeo via código React/Node.js).
    """
    ...
    def generate_video(self, prompt: str, filename: str = "remotion_render.mp4") -> GeneratedCreativeAsset:
        if not self._enabled:
            raise RuntimeError("RemotionVideoProvider is disabled.")

        seed_hash = hashlib.sha256(f"REMOTION:{prompt}".encode("utf-8")).digest()
